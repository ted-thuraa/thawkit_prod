...
interface CollectionsActions {
  ...
  createField: (
    collectionId: string,
    data: Omit<CreateCollectionFieldData, "collection_id">,
  ) => Promise<CollectionField>;
  updateField: (
    collectionId: string,
    fieldId: string,
    data: UpdateCollectionFieldData,
  ) => Promise<void>;
  ...
  duplicateItem: (
    collectionId: string,
    itemId: string,
  ) => Promise<CollectionItemWithValues | undefined>;
  searchItems: (
    collectionId: string,
    query: string,
    page?: number,
  ...
  updateCollectionSorting: (
    collectionId: string,
    sorting: { field: string; direction: "asc" | "desc" | "manual" },
  ) => Promise<void>;
  reorderItems: (
    collectionId: string,
    updates: Array<{ id: string; manual_order: number }>,
  ) => Promise<void>;
...
export const useCollectionsStore = create<CollectionsStore>((set, get) => ({
  ...
  preloadCollectionsAndItems: async (collections: Collection[]) => {
    ...
    collections.forEach((collection) => {
      ...
      const needsServerSort =
        sorting && sorting.direction !== "manual" && totalCount > PRELOAD_LIMIT;
      if (needsServerSort) {
    ...
    sortedCollections_.forEach((collection, i) => {
      ...
      queryMap[collection.id] = {
        page: 1,
        limit: PRELOAD_LIMIT,
        ...getSortParams(collection.sorting),
      };
    });
      ...
      try {
        ...
        const fieldResponses = await Promise.all(
          builtInFields.map((field) =>
            collectionsApi.createField(newCollection.id, field),
          ),
        );
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to create sample collection";
      set({ error: errorMessage, isLoading: false });
      throw error;
    }
  },
  ...
  deleteCollection: async (id) => {
    ...
    try {
      ...
      set((state) => {
        ...
        const newSelectedId =
          wasSelected && sortedCollections.length > 0
  ...
  updateField: async (collectionId, fieldId, data) => {
    ...
    try {
      const response = await collectionsApi.updateField(
        collectionId,
        fieldId,
        data,
      );
      ...
      set((state) => ({
        fields: {
          ...state.fields,
          [collectionId]: (state.fields[collectionId] || []).map((f) =>
  ...
  deleteField: async (collectionId, fieldId) => {
    ...
    try {
      ...
      set((state) => ({
        fields: {
          ...state.fields,
          [collectionId]: (state.fields[collectionId] || []).filter(
            (f) => f.id !== fieldId,
          ),
        },
        isLoading: false,
  ...
  loadItems: async (
    ...
    try {
      ...
      if (response.data?.referencedAssets?.length) {
        useAssetsStore
          .getState()
          .addAssetsToCache(response.data.referencedAssets);
      }
  ...
  createItem: async (collectionId, values, statusAction) => {
    ...
    if (statusFieldId) {
      optimisticValues[statusFieldId] = buildStatusValue(
        isPublishable,
        isPublished,
      );
    }
    ...
    try {
      const response = await collectionsApi.createItem(
        collectionId,
        values,
        statusAction,
      );
      ...
      set((state) => ({
        ...
        itemsTotalCount: {
          ...
          [collectionId]: Math.max(
            0,
            (state.itemsTotalCount[collectionId] || 0) - 1,
          ),
        },
  ...
  updateItem: async (collectionId, itemId, values) => {
    ...
    if (statusFieldId && previousItem) {
      try {
        ...
        if (current.is_published) {
          optimisticStatus[statusFieldId] = buildStatusValue(
            current.is_publishable,
            true,
            true,
          );
        }
      } catch {
        /* non-JSON status value, skip */
      }
    }
    ...
    const updatedAtFieldId = collectionFields.find(
      (f) => f.key === "updated_at",
    )?.id;
    ...
    try {
      const response = await collectionsApi.updateItem(
        collectionId,
        itemId,
        values,
      );
  ...
  duplicateItem: async (collectionId: string, itemId: string) => {
    ...
    const sourceItem = get().items[collectionId]?.find(
      (item) => item.id === itemId,
    );
    ...
    if (sourceItem) {
      ...
      const createdAtField = collectionFields.find(
        (f) => f.key === "created_at",
      );
      const updatedAtField = collectionFields.find(
        (f) => f.key === "updated_at",
      );
      ...
      if (slugField) {
        const originalSlug = newValues[slugField.id]
          ? String(newValues[slugField.id]).trim()
          : "";
      ...
      const dupStatusFieldId = findStatusFieldId(
        get().fields[collectionId] || [],
      );
      if (dupStatusFieldId) {
  ...
  setItemPublishable: async (
    ...
    try {
      const response = await collectionsApi.setItemPublishable(
        collectionId,
        itemId,
        isPublishable,
      );
  ...
  setItemStatus: async (
    ...
    try {
      const response = await collectionsApi.setItemStatus(
        collectionId,
        itemId,
        action,
      );
  ...
  getDropdownItems: async (collectionId: string) => {
    try {
      ...
      if (!nameField) {
        console.warn(
          `Name field not found for collection ${collectionId}. Available fields:`,
          collectionFields.map((f) => ({ id: f.id, key: f.key, name: f.name })),
        );
      }

      return items.map((item) => ({
        id: item.id,
        label: getCollectionItemLabel(item, collectionFields),
      }));
    } catch (error) {
  ...
  searchAndMergeItems: async (
    ...
    try {
      ...
      return fetched.map((item) => ({
        id: item.id,
        label: getCollectionItemLabel(item, fields),
      }));
    } catch (error) {
