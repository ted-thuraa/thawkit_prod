...
import { Layer, LayerTemplate, LayerTemplateRef } from "@/types/funnel";
  ...
  const assignIds = (layer: Omit<Layer, "id">): Layer => {
    ...
    if (layerWithId.children && Array.isArray(layerWithId.children)) {
      layerWithId.children = layerWithId.children.map((child) =>
        assignIds(child as Omit<Layer, "id">),
      ) as Layer[];
    }
...
export function getTemplateRef(
  index: string,
  overrides?: Partial<LayerTemplate>,
): LayerTemplateRef {
...
export function getLayoutsByCategory(): Record<string, string[]> {
  ...
  Object.keys(layoutTemplates).forEach((key) => {
    const category =
      layoutTemplates[key as keyof typeof layoutTemplates].category || "Other";
