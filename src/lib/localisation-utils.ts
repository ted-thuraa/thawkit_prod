...
import {
  ...
  createDynamicRichTextVariable,
  createDynamicRichTextVariableFromPlainText,
  createAssetVariable,
} from "@/lib/variable-utils";
import { castValue } from "@/lib/collection-utils";
import {
  tiptapDocHasFormatting,
  tiptapDocToCanonicalString,
  hasVariableNode,
  hasAnyTextOrVariable,
} from "@/lib/tiptap-utils";
import { stringToTiptapContent } from "@/lib/text-format-utils";
import { isRichTextLayer } from "@/lib/layer-utils";
import type { IconProps } from "@/components/ui/icon";
import { looksLikeFormattedHtml } from "./translation-classification";
...
export const LOCALES: LocaleOption[] = [
  ...
  { code: "ar", label: "Arabic", native_label: "العربية", rtl: true },
  {
    code: "ar-eg",
    label: "Arabic (Egypt)",
    native_label: "العربية (مصر)",
    rtl: true,
  },
  {
    code: "ar-lb",
    label: "Arabic (Lebanon)",
    native_label: "العربية (لبنان)",
    rtl: true,
  },
  {
    code: "ar-ma",
    label: "Arabic (Morocco)",
    native_label: "العربية (المغرب)",
    rtl: true,
  },
  {
    code: "ar-sa",
    label: "Arabic (Saudi Arabia)",
    native_label: "العربية (السعودية)",
    rtl: true,
  },
  ...
  { code: "en", label: "English", native_label: "English" },
  {
    ...
    native_label: "English (Australia)",
  },
  {
    code: "en-ca",
    label: "English (Canada)",
    native_label: "English (Canada)",
  },
  { code: "en-in", label: "English (India)", native_label: "English (India)" },
  {
    code: "en-nz",
    label: "English (New Zealand)",
    native_label: "English (New Zealand)",
  },
  {
    code: "en-gb",
    label: "English (United Kingdom)",
    native_label: "English (United Kingdom)",
  },
  {
    code: "en-us",
  ...
  { code: "de", label: "German", native_label: "Deutsch" },
  {
    ...
    native_label: "Deutsch (Österreich)",
  },
  {
    code: "de-de",
  ...
  { code: "ia", label: "Interlingua", native_label: "Interlingua" },
  {
    code: "ie",
    label: "Interlingue",
    native_label: "Interlingue (Occidental)",
  },
  ...
  },
  {
    ...
    native_label: "Português (Portugal)",
  },
  ...
  { code: "es", label: "Spanish", native_label: "Español" },
  {
    ...
    native_label: "Español (Argentina)",
  },
  { code: "es-cl", label: "Spanish (Chile)", native_label: "Español (Chile)" },
  {
    code: "es-co",
    label: "Spanish (Colombia)",
    native_label: "Español (Colombia)",
  },
  {
    code: "es-mx",
    label: "Spanish (Mexico)",
    native_label: "Español (México)",
  },
...
const LOCALES_BY_CODE = new Map<string, LocaleOption>(
...
export function parseLocaleCode(code: string): {
  ...
  return {
    language: code.slice(0, dashIndex),
    region: code.slice(dashIndex + 1),
  };
}
...
export function extractImageAltText(layer: Layer): string | null {
  ...
  if (
    !layer.variables?.image?.alt ||
    layer.variables.image.alt.type !== "dynamic_text"
  ) {
    return null;
  }
...
function classifyTextVariableForTranslation(
  ...
  if (textVariable.type === "dynamic_text") {
    ...
    if (
      looksLikeFormattedHtml(text) ||
      text.includes("<ycode-inline-variable>")
    ) {
      return {
        contentType: "richtext",
        value: text.trim(),
        openInSheet: isRichSource,
      };
    }
...
function classifyLayerTextForTranslation(layer: Layer): {
  contentType: "text" | "richtext";
  value: string;
  openInSheet: boolean;
} | null {
  ...
  return classifyTextVariableForTranslation(
    layer.variables?.text as any,
...
function extractMediaTranslatableItems(
  ...
  if (layer.name === "video" && layer.variables?.video) {
    ...
    if (
      videoPoster &&
      videoPoster.type === "asset" &&
      videoPoster.data?.asset_id
    ) {
      items.push({
...
function extractComponentOverrideTranslatableItems(
  ...
  for (const category of ["text", "rich_text"] as const) {
    ...
    for (const [varId, value] of Object.entries(catOverrides)) {
      ...
      items.push({
        ...
        info: buildInfo(
          variableName,
          classification.contentType === "richtext" ? "type" : "text",
        ),
      });
    }
  }
  ...
  if (imageOverrides) {
    for (const [varId, value] of Object.entries(imageOverrides)) {
      ...
      if (
        alt &&
        alt.type === "dynamic_text" &&
        typeof alt.data?.content === "string" &&
        alt.data.content.trim()
      ) {
        items.push({
...
function extractLayerTranslatableItems(
  ...
  for (const layer of layers) {
    ...
    if (layer.name === "localeSelector" || layer.key === "localeSelectorLabel")
      continue;
    ...
    if (
      ...
      extractComponentOverrideTranslatableItems(
        layer,
        sourceId,
        componentsById,
        items,
 ...
 */
function classifySeoValue(value: string): {
  contentType: "text" | "richtext";
  openInSheet: boolean;
} {
  if (
    looksLikeFormattedHtml(value) ||
    value.includes("<ycode-inline-variable>")
  ) {
  ...
  for (const field of fields) {
    ...
    items.push({
      ...
      info: {
        ...
        label: field.name,
        description: isSlugField
          ? `Affects ${localeName} URLs generated by dynamic pages using this CMS item`
          : undefined,
      },
    });
  }
...
export function injectTranslatedText(
  ...
  const valueOptions = options?.includeIncomplete
  ...
  return layers.map((layer) => {
    ...
    const masterComponentId =
      ((layer as any)._masterComponentId as string | undefined) ??
      options?.defaultMasterComponentId;

    // 1. Inject text translation
    const textTranslationKey = buildLayerTranslationKey(
      pageId,
      `layer:${translationLayerId}:text`,
      masterComponentId,
    );
    const textTranslation = getTranslationByKey(
      translations,
      textTranslationKey,
    );
    ...
    const textReintroducesDynamicVariable =
      richTextHasDynamicVariable(textValue);
    ...
    if (layer.name === "image" && !(layer as any)._imageFromOverride) {
      const imageSrcKey = buildLayerTranslationKey(
        pageId,
        `layer:${translationLayerId}:image_src`,
        masterComponentId,
      );
      const imageSrcTranslation = getTranslationByKey(
        translations,
        imageSrcKey,
      );
      const imageAltKey = buildLayerTranslationKey(
        pageId,
        `layer:${translationLayerId}:image_alt`,
        masterComponentId,
      );
      const imageAltTranslation = getTranslationByKey(
        translations,
        imageAltKey,
      );
      ...
      if (imageSrcTranslation || imageAltTranslation) {
        ...
        if (imageSrcTranslation && imageSrcTranslation.content_value) {
          imageUpdates.src = createAssetVariable(
            imageSrcTranslation.content_value,
          );
        }

        const imageAltValue = getTranslationValue(
          imageAltTranslation,
          valueOptions,
        );
        if (imageAltValue) {
          ...
          imageUpdates.alt =
            layer.variables?.image?.alt || createDynamicTextVariable("");
        }
    ...
    if (layer.name === "video") {
      const videoSrcKey = buildLayerTranslationKey(
        pageId,
        `layer:${translationLayerId}:video_src`,
        masterComponentId,
      );
      const videoSrcTranslation = getTranslationByKey(
        translations,
        videoSrcKey,
      );
      const videoPosterKey = buildLayerTranslationKey(
        pageId,
        `layer:${translationLayerId}:video_poster`,
        masterComponentId,
      );
      const videoPosterTranslation = getTranslationByKey(
        translations,
        videoPosterKey,
      );
      ...
      if (videoSrcTranslation || videoPosterTranslation) {
        ...
        if (videoSrcTranslation && videoSrcTranslation.content_value) {
          videoUpdates.src = createAssetVariable(
            videoSrcTranslation.content_value,
          );
        }

        if (videoPosterTranslation && videoPosterTranslation.content_value) {
          videoUpdates.poster = createAssetVariable(
            videoPosterTranslation.content_value,
          );
        }
    ...
    if (layer.name === "audio") {
      const audioSrcKey = buildLayerTranslationKey(
        pageId,
        `layer:${translationLayerId}:audio_src`,
        masterComponentId,
      );
      const audioSrcTranslation = getTranslationByKey(
        translations,
        audioSrcKey,
      );
    ...
    if (layer.children && layer.children.length > 0) {
      updates.children = injectTranslatedText(
        layer.children,
        pageId,
        translations,
        options,
...
export function translateComponentOverrides(
  ...
  const walk = (list: Layer[]): Layer[] =>
    list.map((layer) => {
      const overrides = layer.componentOverrides;
      const nextChildren = layer.children?.length
        ? walk(layer.children)
        : layer.children;
      if (!overrides) {
        return nextChildren === layer.children
          ? layer
      ...
      for (const category of ["text", "rich_text"] as const) {
        ...
        for (const [varId, value] of Object.entries(catOverrides)) {
          ...
          if (
            ...
            const content =
              val.type === "dynamic_rich_text"
                ? looksLikeTiptapJson(v)
                  ? safeParseJson(v)
                  : stringToTiptapContent(v)
                : v;
      ...
      if (imageOverrides) {
        ...
        for (const [varId, value] of Object.entries(imageOverrides)) {
          const srcAssetId = lookup(
            `layer:${layer.id}:override:image_src:${varId}`,
          );
          const altText = lookup(
            `layer:${layer.id}:override:image_alt:${varId}`,
          );
          if (
            (srcAssetId === undefined && altText === undefined) ||
            !value ||
            typeof value !== "object"
          ) {
          ...
          nextImage[varId] = {
            ...
            ...(srcAssetId !== undefined && src
              ? {
                  src: {
                    ...src,
                    data: { ...(src.data || {}), asset_id: srcAssetId },
                  },
                }
              : {}),
            ...(altText !== undefined
 ...
 * resolution pipeline.
 */
export function richTextHasDynamicVariable(
