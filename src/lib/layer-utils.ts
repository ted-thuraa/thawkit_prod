 ...
 */
export function parseStyleStringToObject(
  style: string,
...
export function applyCustomAttributes(
  ...
  for (const [name, value] of Object.entries(customAttributes)) {
    ...
    if (jsxName === "style" && typeof value === "string") {
      const existing = (
        typeof target.style === "object" && target.style ? target.style : {}
...
export function indexedFindLayerById(
...
export function indexedFindLayerWithParent(
  indexes: LayerIndexes,
  targetId: string,
): { layer: Layer; parent: Layer | null } | null {
...
export function indexedFindParentCollectionLayer(
  indexes: LayerIndexes,
  layerId: string,
): Layer | null {
...
export function indexedFindAncestor(
  indexes: LayerIndexes,
  layerId: string,
 ...
 */
export function hasPasswordFormLayer(
  layers: Layer[] | undefined | null,
): boolean {
  if (!layers || layers.length === 0) return false;
  for (const layer of layers) {
    if (
      layer.name === "form" &&
      layer.settings?.form?.form_type === "password_protected"
    ) {
      return true;
    }
 ...
 */
export function filterDisabledSliderLayers(
  layers: Layer[],
  sliderSettings?: Layer["settings"],
): Layer[] {
  ...
  const inputLayers = sliderConfig
    ? layers.filter((layer) => {
        if (layer.name === "slideNavigationWrapper" && !sliderConfig.navigation)
          return false;
        if (layer.name === "slidePaginationWrapper" && !sliderConfig.pagination)
          return false;
        return true;
      })
    : layers;
  ...
  return inputLayers.map((layer) => {
    ...
    const currentSliderSettings =
      layer.name === "slider" ? layer.settings : sliderSettings;
    let filteredChildren = filterDisabledSliderLayers(
      layer.children,
    ...
    return filteredChildren === layer.children
      ? layer
...
export function canMoveLayer(
  ...
  if (newParentId !== null) {
    ...
    const hasLinkAncestor = findAncestor(layers, newParentId, (ancestor) =>
      layerHasLink(ancestor),
    );
    ...
    for (const layer of searchLayers) {
      if (layer.id === layerId) {
        ...
        if (ancestors.some((a) => a.name === "filter")) {
          if (layer.children?.some((c) => FILTER_INPUT_TYPES.includes(c.name)))
            return true;
          const parent = ancestors[ancestors.length - 1];
          if (
            parent?.children?.some((c) => FILTER_INPUT_TYPES.includes(c.name))
          )
            return true;
        }
        return false;
...
export function resolveFilterInputId(layerId: string, layers: Layer[]): string {
  ...
  if (layer.children) {
    const inputChild = layer.children.find((c) =>
      FILTER_INPUT_TYPES.includes(c.name),
    );
  ...
  const findParent = (searchLayers: Layer[]): Layer | null => {
    for (const l of searchLayers) {
      ...
      if (l.children) {
        const found = findParent(l.children);
  ...
  if (parent?.children) {
    const siblingInput = parent.children.find((c) =>
      FILTER_INPUT_TYPES.includes(c.name),
    );
 ...
 */
export function getFilterInputLayers(layers: Layer[]): Array<{
  layerId: string;
  layerName: string;
  customName?: string;
  ...
  const walk = (searchLayers: Layer[], filterLayerId: string | null) => {
    for (const layer of searchLayers) {
      const currentFilterId =
        layer.name === "filter" ? layer.id : filterLayerId;
...
export function containsLayerId(layer: Layer, targetId: string): boolean {
  ...
  return (
    layer.children?.some((child) => containsLayerId(child, targetId)) ?? false
  );
}

/** Collect every layer id in a tree into a Set (used to diff trees). */
export function collectLayerIdSet(
  layers: Layer[],
 ...
 */
export function findAddedLayerIds(
  oldLayers: Layer[],
  newLayers: Layer[],
): string[] {
 ...
 */
export function generateUniqueSettingsId(
  baseId: string,
 ...
 */
export function findParentCollectionLayer(
  layers: Layer[],
  layerId: string,
): Layer | null {
  // Helper to find a layer and its parent chain
  const findLayerWithParents = (
    layers: Layer[],
    targetId: string,
    parent: Layer | null = null,
  ): { layer: Layer; parent: Layer | null } | null => {
 ...
 */
export function findAllParentCollectionLayers(
  layers: Layer[],
  layerId: string,
): Layer[] {
  const result: Layer[] = [];
...
export function getTextHeadingConversion(
  ...
  const clearLabel =
    !!layer.customName && AUTO_TEXT_HEADING_LABELS.has(layer.customName);
 ...
 */
export function contentBlockToStyleKey(block: {
  type: string;
...
export function hasRichTextContent(layer: Layer): boolean {
  ...
  return layerDoc.content.some(
    (block: any) => block.type !== "paragraph" || block.content?.length,
  );
}

export function getRichTextSublayers(
  layer: Layer,
  cmsContent?: any,
): RichTextSublayer[] {
  const textVar = layer.variables?.text;
...
function buildSublayersFromDoc(doc: any, layer: Layer): RichTextSublayer[] {
  return doc.content
    ...
    .map((block: any) => {
      ...
      INLINE_STYLE_KEYS.filter((k) => marks.includes(k)).forEach((markType) => {
        children.push({
          type: markType,
 ...
 */
function buildUniqueStyleSublayersFromDoc(
  doc: any,
  layer: Layer,
): RichTextSublayer[] {
    ...
    for (const block of blocks) {
      ...
      if (styleKey && !seenStyleKeys.has(styleKey)) {
        ...
        sublayers.push({
          ...
          icon:
            STYLE_SUBLAYER_ICON_MAP[styleKey] ||
            SUBLAYER_ICON_MAP[block.type] ||
            "box",
          kind: "style" as const,
          styleKey,
        });
      ...
      if (
        isList &&
        !seenStyleKeys.has("listItem") &&
        block.content?.some((c: any) => c.type === "listItem")
      ) {
  ...
  collectBlockTypes(
    doc.content.filter((b: any) => b.type !== "paragraph" || b.content?.length),
  );

  // Collect unique inline marks across all blocks
  const allMarks = extractInlineMarks(doc);
  INLINE_STYLE_KEYS.filter((k) => allMarks.includes(k)).forEach((markType) => {
    sublayers.push({
      type: markType,
...
function buildAllStyleSublayers(layer: Layer): RichTextSublayer[] {
  ...
  return SUBLAYER_SORT_ORDER.map((styleKey) => ({
    ...
    label: allStyles[styleKey]?.label || styleKey,
    icon:
      STYLE_SUBLAYER_ICON_MAP[styleKey] || SUBLAYER_ICON_MAP[styleKey] || "box",
    kind: "style" as const,
    styleKey,
  }));
}
...
export function getTextStyleSublayers(layer: Layer): RichTextSublayer[] {
  ...
  const textVar = layer.variables?.text;
  const doc =
    textVar?.type === "dynamic_rich_text"
      ? (textVar.data as any)?.content
      : null;
  ...
  return INLINE_STYLE_KEYS.filter((key) => usedMarks.includes(key)).map(
    (key) => ({
      type: key,
 ...
 */
export function removeRichTextSublayer(
  layer: Layer,
...
export function canLayerHaveLink(
  ...
  issue?: {
    type: "self" | "ancestor" | "child" | "richText" | "unsupported";
    layerName?: string;
  };
} {
  if (type === "layer") {
    ...
    const hasAncestorWithLink = findAncestor(allLayers, layer.id, (ancestor) =>
      hasLinkSettings(ancestor),
    );
    ...
    if (hasAncestorWithLink) {
      return {
        ...
        issue: {
          type: "ancestor",
          layerName: layerLabelFallback(hasAncestorWithLink),
        },
      };
    }
    ...
    if (layer.children && layer.children.length > 0) {
      const hasChildWithLink = layer.children.some((child) =>
        hasLinkInTree(child),
      );
      if (hasChildWithLink) {
        return {
...
export const LINK_NESTING_ERROR = {
  ...
  description:
    "The pasted layer contains a link and cannot be placed inside a link.",
} as const;
...
export function canPasteIntoParent(
  ...
  const hasLinkAncestor = findAncestor(layers, parentId, (ancestor) =>
    layerHasLink(ancestor),
  );
...
export function reorderSiblings(
  ...
  const findParentWithChild = (
    ...
    for (let i = 0; i < layerList.length; i++) {
      ...
      if (layerList[i].children) {
        const found = findParentWithChild(
          layerList[i].children!,
          childId,
          layerList[i],
        );
        if (found) return found;
  ...
  if (movedParentId !== targetParentId) {
    console.warn(
      "[reorderSiblings] Layers have different parents - cannot reorder",
    );
    return layers;
  }
...
export function getTextWithBinding(
  ...
  if (textVariable && textVariable.type === "dynamic_text") {
    ...
    if (content.includes("<ycode-inline-variable>")) {
      ...
      return resolveInlineVariablesFromData(
        content,
        collectionItemData,
        null,
        timezone,
...
export function getLayoutTypeForBreakpoint(
  ...
  const { value: displayClass } = getInheritedValue(
    classes,
    "display",
    breakpoint,
  );
  const { value: flexDirectionClass } = getInheritedValue(
    classes,
    "flexDirection",
    breakpoint,
  );
 ...
 */
export function getLayerCmsFieldBinding(
  layer: Layer,
): CmsFieldBindingInfo | null {
  ...
  const extractBinding = (v: any): CmsFieldBindingInfo | null => {
    if (v && typeof v === "object" && v.type === "field" && v.data?.field_id) {
      return {
        field_id: v.data.field_id,
        source: v.data.source,
        collection_layer_id: v.data.collection_layer_id,
      };
    }
    return null;
  };
  ...
  if (vars.text?.type === "dynamic_text") {
    ...
    const match = content.match(
      /<ycode-inline-variable>([\s\S]*?)<\/ycode-inline-variable>/,
    );
    if (match) {
      try {
 ...
 */
export function findAllCollectionLayers(
  layers: Layer[],
  ...
  const traverse = (
    layerList: Layer[],
    foundCollectionInBranch: boolean = false,
  ) => {
    for (const layer of layerList) {
...
export function evaluateCondition(
  condition: import("@/types/funnel").VisibilityCondition,
  ...
  if (condition.source === "page_collection") {
    ...
    const count =
      pageCollectionCounts?.[condition.collectionLayerId || ""] ?? 0;
  ...
  if (condition.source === "collection_field") {
    ...
    // checks) expects.
    const rawValue = resolveFieldFromSources(
      fieldId,
      undefined,
      collectionLayerData,
      pageCollectionData,
    ...
    if (condition.valueMode === "current_page") {
      ...
      if (isReferenceField) {
        ...
        compareValue = String(
          pageCollectionData?.[condition.currentPageFieldId] ?? "",
        );
      }
    }
    ...
    switch (effectiveOperator) {
      ...
      case "is":
        if (fieldType === "boolean") {
          ...
          return (
            normalizeBooleanValue(value) === normalizeBooleanValue(compareValue)
          );
        }
        if (fieldType === "number") {
      ...
      case "contains_exactly": {
        try {
          ...
          return (
            requiredIds.length === valueIds.length &&
      ...
      case "item_count": {
        if (
          condition.source === "collection_field" &&
          condition.fieldType === "multi_reference"
        ) {
          let count = 0;
          try {
...
export function evaluateVisibility(
  conditionalVisibility:
    | import("@/types/funnel").ConditionalVisibility
    | undefined,
  context: VisibilityContext,
): boolean {
  ...
  if (
    !conditionalVisibility ||
    !conditionalVisibility.groups ||
    conditionalVisibility.groups.length === 0
  ) {
    return true;
  }
...
function buildComponentMap(
  ...
  currentComponentRootId: string | null = null,
): Record<string, string> {
 ...
 */
function tagLayerSubtreeWithComponentId(
  layer: Layer,
  componentId: string,
): Layer {
  return {
    ...
    children: layer.children
      ? layer.children.map((child) =>
          tagLayerSubtreeWithComponentId(child, componentId),
        )
      : layer.children,
  };
}
  ...
  return effectiveLayers.map((layer) => {
    ...
    if (layer.componentId) {
      ...
      if (visited.has(layer.componentId)) {
        console.warn(
          "[resolveComponentsInLayers] Circular component reference detected, skipping:",
          layer.componentId,
        );
    ...
    if (layer.children && layer.children.length > 0) {
      return {
        ...layer,
        children: resolveComponentsInLayers(
          layer.children,
          components,
...
export function serializeLayers(
  ...
  const resolvedLayers = resolveComponentsInLayers(
    layers,
    components,
    ...
    return layerList.map((layer) => {
      if (
        ...
        for (const config of breakpointConfigs) {
          const hasOrderedSiblings = siblings.some((c) =>
            hasOrderClassForPrefix(c.classes, config.prefix),
          );
          ...
          if (hasOrderedSiblings) {
            ...
            siblings.forEach((sibling) => {
              const orderValue = getOrderValueForPrefix(
                sibling.classes,
                config.prefix,
              );
...
export function replaceLayerWithComponentInstance(
  ...
  return layers.map((layer) => {
    ...
    if (layer.children) {
      return {
        ...layer,
        children: replaceLayerWithComponentInstance(
          layer.children,
          layerId,
          componentId,
        ),
 ...
 */
function stripInvalidInlineVariables(
  text: string,
  ctx: CollectionContext,
): string | null {
  if (!text) return null;
 ...
 */
function cleanDesignColorVariable(
  dcv: DesignColorVariable,
  ctx: CollectionContext,
): DesignColorVariable | undefined {
  let changed = false;
  const result = { ...dcv };
  ...
  if (
    result.field &&
    isCollectionBoundField(result.field) &&
    !isFieldVariableValid(result.field, ctx)
  ) {
  ...
  if (result.linear?.stops) {
    const cleanedStops = result.linear.stops.map((stop: BoundColorStop) => {
      if (
        stop.field &&
        isCollectionBoundField(stop.field) &&
        !isFieldVariableValid(stop.field, ctx)
      ) {
        changed = true;
 ...
 */
function resetLayerVariableBindings(
  variables: LayerVariables | undefined,
  ctx: CollectionContext,
): LayerVariables | null {
  ...
  if (updated.text) {
    if (
      updated.text.type === "dynamic_text" &&
      typeof updated.text.data?.content === "string"
    ) {
      const cleaned = stripInvalidInlineVariables(
    ...
    if (
      updated.text.type === "dynamic_rich_text" &&
      typeof updated.text.data?.content === "object"
    ) {
  ...
  if (updated.image) {
    ...
    if (
      updated.image.alt?.type === "dynamic_text" &&
      typeof updated.image.alt.data?.content === "string"
    ) {
      const cleaned = stripInvalidInlineVariables(
        updated.image.alt.data.content,
        ctx,
      );
      if (cleaned !== null) {
        updated.image = {
          ...updated.image,
          alt: { ...updated.image.alt, data: { content: cleaned } },
        };
        changed = true;
      }
    }
  ...
  if (updated.audio?.src) {
    ...
    if (
      fv.type === "field" &&
      isCollectionBoundField(fv as FieldVariable) &&
      !isFieldVariableValid(fv as FieldVariable, ctx)
    ) {
      updated.audio = {
        ...updated.audio,
        src: { type: "asset", data: { asset_id: null } },
  ...
  if (
    updated.iframe?.src?.type === "dynamic_text" &&
    typeof updated.iframe.src.data?.content === "string"
  ) {
    const cleaned = stripInvalidInlineVariables(
      updated.iframe.src.data.content,
      ctx,
    );
    if (cleaned !== null) {
      updated.iframe = {
        ...updated.iframe,
        src: { ...updated.iframe.src, data: { content: cleaned } },
      };
      changed = true;
    }
  }
  ...
  if (updated.link) {
    ...
    if (
      updated.link.url?.type === "dynamic_text" &&
      typeof updated.link.url.data?.content === "string"
    ) {
      const cleaned = stripInvalidInlineVariables(
        updated.link.url.data.content,
        ctx,
      );
      if (cleaned !== null) {
        updated.link = {
          ...updated.link,
          url: { ...updated.link.url, data: { content: cleaned } },
        };
        changed = true;
      }
    }
    ...
    if (
      updated.link.field &&
      isCollectionBoundField(updated.link.field) &&
      !isFieldVariableValid(updated.link.field, ctx)
    ) {
    ...
    if (
      updated.link.email?.type === "dynamic_text" &&
      typeof updated.link.email.data?.content === "string"
    ) {
      const cleaned = stripInvalidInlineVariables(
        updated.link.email.data.content,
        ctx,
      );
      if (cleaned !== null) {
        updated.link = {
          ...updated.link,
          email: { ...updated.link.email, data: { content: cleaned } },
        };
        changed = true;
      }
    }
    ...
    if (
      updated.link.phone?.type === "dynamic_text" &&
      typeof updated.link.phone.data?.content === "string"
    ) {
      const cleaned = stripInvalidInlineVariables(
        updated.link.phone.data.content,
        ctx,
      );
      if (cleaned !== null) {
        updated.link = {
          ...updated.link,
          phone: { ...updated.link.phone, data: { content: cleaned } },
        };
        changed = true;
      }
    }
  ...
  if (updated.design) {
    ...
    for (const key of designKeys) {
      ...
      if (dcv) {
        ...
        if (cleaned !== dcv) {
          (newDesign as Record<string, DesignColorVariable | undefined>)[key] =
            cleaned;
          designChanged = true;
        }
      }
  ...
  const cleanedBlocks = doc.content.map((block: any) => {
    ...
    const cleanedNodes = block.content.filter((node: any) => {
      if (node.type === "dynamicVariable" && node.attrs?.variable) {
        ...
        if (
          fv.type === "field" &&
          isCollectionBoundField(fv) &&
          !isFieldVariableValid(fv, ctx)
        ) {
          changed = true;
          return false;
        }
 ...
 */
export function resetBindingsAfterMove(
  layers: Layer[],
  movedLayerId: string,
): Layer[] {
...
function stripPageSourceBindings(layers: Layer[]): Layer[] {
  return layers.map((layer) => {
    ...
    if (layer.children && layer.children.length > 0) {
      const cleanedChildren = stripPageSourceBindings(
 ...
 */
function resetOrphanedCollectionSources(
  subtree: Layer[],
  fullTree: Layer[],
): Layer[] {
  return subtree.map((layer) => {
...
function stripPageSourceInlineVariables(text: string): string | null {
  ...
  const result = text.replace(INLINE_VAR_REGEX, (match, content) => {
    try {
      ...
      if (
        parsed.type === "field" &&
        isPageBoundField(parsed as FieldVariable)
      ) {
        changed = true;
        return "";
      }
...
function stripPageSourceFromDesignColor(
  dcv: DesignColorVariable,
): DesignColorVariable | undefined {
  let changed = false;
  const result = { ...dcv };
...
function stripPageSourceFromVariables(
  ...
  if (updated.conditionalVisibility?.groups) {
    ...
    const cleanedGroups = updated.conditionalVisibility.groups.map((group) => {
      ...
      return cleanedConditions.length !== group.conditions.length
        ? { ...group, conditions: cleanedConditions }
        : group;
    });
    if (cvChanged) {
      updated.conditionalVisibility = { groups: cleanedGroups };
  ...
  if (
    updated.image?.src &&
    updated.image.src.type === "field" &&
    isPageBoundField(updated.image.src as FieldVariable)
  ) {
    updated.image = {
      ...updated.image,
      src: { type: "asset", data: { asset_id: null } },
  ...
  if (
    ...
    const cleaned = stripPageSourceInlineVariables(
      updated.image.alt.data.content,
    );
    if (cleaned !== null) {
      updated.image = {
  ...
  // Audio variable
  if (
    updated.audio?.src?.type === "field" &&
    isPageBoundField(updated.audio.src as FieldVariable)
  ) {
    updated.audio = {
      ...updated.audio,
      src: { type: "asset", data: { asset_id: null } },
  ...
  // Video variable
  if (
    updated.video?.src?.type === "field" &&
    isPageBoundField(updated.video.src as FieldVariable)
  ) {
    updated.video = { ...updated.video, src: undefined };
    changed = true;
  }
  if (
    updated.video?.poster?.type === "field" &&
    isPageBoundField(updated.video.poster as FieldVariable)
  ) {
    updated.video = { ...updated.video, poster: undefined };
    changed = true;
  }
  ...
  if (updated.link) {
    ...
    if (
      updatedLink.url?.type === "dynamic_text" &&
      typeof updatedLink.url.data?.content === "string"
    ) {
      const cleaned = stripPageSourceInlineVariables(
        updatedLink.url.data.content,
      );
      if (cleaned !== null) {
    ...
    if (
      updatedLink.email?.type === "dynamic_text" &&
      typeof updatedLink.email.data?.content === "string"
    ) {
      const cleaned = stripPageSourceInlineVariables(
        updatedLink.email.data.content,
      );
      if (cleaned !== null) {
        updatedLink.email = {
          ...updatedLink.email,
    ...
    if (
      updatedLink.phone?.type === "dynamic_text" &&
      typeof updatedLink.phone.data?.content === "string"
    ) {
      const cleaned = stripPageSourceInlineVariables(
        updatedLink.phone.data.content,
      );
      if (cleaned !== null) {
        updatedLink.phone = {
          ...updatedLink.phone,
 ...
 */
export function resetBindingsOnCollectionSourceChange(
  layers: Layer[],
 ...
 */
function stripBindingsForCollectionLayer(
  subtree: Layer[],
  collectionLayerId: string,
): Layer[] {
  ...
  const result = subtree.map((layer) => {
    ...
    if (updated.variables) {
      const cleaned = resetVariablesForCollectionLayer(
        updated.variables,
        collectionLayerId,
      );
      if (cleaned) {
...
function resetCollectionLayerOwnBindings(layer: Layer): Layer {
  ...
  if (updated.variables?.conditionalVisibility?.groups) {
    ...
    const cleanedGroups = updated.variables.conditionalVisibility.groups.map(
      (group) => {
        const cleanedConditions = group.conditions.filter((c) => {
 ...
 */
function fieldReferencesCollectionLayer(
  fv: FieldVariable,
  collectionLayerId: string,
): boolean {
  ...
  if (fv.data?.source === "collection" && !fv.data?.collection_layer_id)
    return true;
 ...
 */
function resetVariablesForCollectionLayer(
  variables: LayerVariables,
  collectionLayerId: string,
): LayerVariables | null {
  let changed = false;
  ...
  if (updated.conditionalVisibility?.groups) {
    ...
    const cleanedGroups = updated.conditionalVisibility.groups.map((group) => {
      const cleanedConditions = group.conditions.filter((c) => {
        if (
          c.source === "page_collection" &&
          c.collectionLayerId === collectionLayerId
        ) {
          visChanged = true;
          return false;
        }
  ...
  if (updated.text) {
    if (
      ...
      const cleaned = stripInlineVarsForCollectionLayer(
        updated.text.data.content,
        collectionLayerId,
      );
      if (cleaned !== null) {
    ...
    if (
      ...
      const cleaned = cleanTiptapForCollectionLayer(
        updated.text.data.content,
        collectionLayerId,
      );
      if (cleaned !== null) {
  ...
  if (updated.image) {
    if (
      updated.image.src?.type === "field" &&
      fieldReferencesCollectionLayer(
        updated.image.src as FieldVariable,
        collectionLayerId,
      )
    ) {
      updated.image = {
  ...
  if (
    updated.audio?.src?.type === "field" &&
    fieldReferencesCollectionLayer(
      updated.audio.src as FieldVariable,
      collectionLayerId,
    )
  ) {
    updated.audio = {
  ...
  if (
    updated.video?.src?.type === "field" &&
    fieldReferencesCollectionLayer(
      updated.video.src as FieldVariable,
      collectionLayerId,
    )
  ) {
    updated.video = { ...updated.video, src: undefined };
  ...
  if (
    updated.video?.poster?.type === "field" &&
    fieldReferencesCollectionLayer(
      updated.video.poster as FieldVariable,
      collectionLayerId,
    )
  ) {
    updated.video = { ...updated.video, poster: undefined };
  ...
  if (updated.link) {
    if (
      updated.link.field &&
      fieldReferencesCollectionLayer(updated.link.field, collectionLayerId)
    ) {
      updated.link = { ...updated.link, type: "url", field: undefined };
      changed = true;
    }
    ...
    for (const key of ["url", "email", "phone"] as const) {
      ...
      if (
        linkVar?.type === "dynamic_text" &&
        typeof linkVar.data?.content === "string"
      ) {
        const cleaned = stripInlineVarsForCollectionLayer(
          linkVar.data.content,
          collectionLayerId,
        );
        if (cleaned !== null) {
          updated.link = {
            ...updated.link,
            [key]: { ...linkVar, data: { content: cleaned } },
          };
          changed = true;
        }
      }
  ...
  if (updated.design) {
    ...
    for (const key of designKeys) {
      const dcv = newDesign[key];
      if (dcv) {
        const cleaned = cleanDesignColorForCollectionLayer(
          dcv,
          collectionLayerId,
        );
        if (cleaned !== dcv) {
...
function stripInlineVarsForCollectionLayer(
  text: string,
  collectionLayerId: string,
): string | null {
  if (!text) return null;
...
function cleanTiptapForCollectionLayer(
  content: object,
  collectionLayerId: string,
): object | null {
  if (!content || typeof content !== "object") return null;
  ...
  const cleanedBlocks = doc.content.map((block: any) => {
    ...
    const cleanedNodes = block.content.filter((node: any) => {
      if (node.type === "dynamicVariable" && node.attrs?.variable) {
        ...
        if (
          fv.type === "field" &&
          fieldReferencesCollectionLayer(fv, collectionLayerId)
        ) {
          changed = true;
          return false;
        }
    ...
    return cleanedNodes.length !== block.content.length
      ? { ...block, content: cleanedNodes }
      : block;
  });
...
function cleanDesignColorForCollectionLayer(
  dcv: DesignColorVariable,
  collectionLayerId: string,
): DesignColorVariable {
  let changed = false;
  ...
  if (
    ...
    fieldReferencesCollectionLayer(result.field, collectionLayerId)
  ) {
    result.field = undefined;
    changed = true;
  }
  ...
  if (result.linear?.stops) {
    const cleanedStops = result.linear.stops.map((stop) => {
      if (
        stop.field?.type === "field" &&
        fieldReferencesCollectionLayer(stop.field, collectionLayerId)
      ) {
        changed = true;
        const { field: _, ...rest } = stop;
        return rest;
 ...
 * @returns Updated layer tree
 */
export function resetBindingsForDeletedCollection(
  layers: Layer[],
  ...
  return layers.map((layer) => {
    ...
    if (updated.variables) {
      const cleanedVars = resetFieldsForDeletedCollection(
        updated.variables,
        deletedCollectionId,
      );
      if (cleanedVars) {
 ...
 * @returns Updated layer tree
 */
export function resetBindingsForDeletedField(
  layers: Layer[],
  ...
  return layers.map((layer) => {
    ...
    if (updated.variables) {
      const cleanedVars = resetVariablesForDeletedField(
        updated.variables,
        deletedFieldId,
      );
      if (cleanedVars) {
 ...
 * Returns updated variables or null if unchanged.
 */
function resetVariablesForDeletedField(
  variables: LayerVariables,
  deletedFieldId: string,
): LayerVariables | null {
  let changed = false;
  ...
  if (updated.text) {
    if (
      ...
      const cleaned = stripInlineVariablesForDeletedField(
        updated.text.data.content,
        deletedFieldId,
      );
      if (cleaned !== null) {
    ...
    if (
      ...
      const cleaned = cleanTiptapContentForDeletedField(
        updated.text.data.content,
        deletedFieldId,
      );
      if (cleaned !== null) {
  ...
  if (updated.design) {
    ...
    for (const dKey of designKeys) {
      ...
      if (dcv) {
        ...
        if (cleaned !== dcv) {
          (newDesign as Record<string, DesignColorVariable | undefined>)[dKey] =
            cleaned;
          designChanged = true;
        }
      }
 ...
 */
function stripInlineVariablesForDeletedField(
  text: string,
  deletedFieldId: string,
): string | null {
  if (!text) return null;
  ...
  const result = text.replace(INLINE_VAR_REGEX, (match, content) => {
    try {
      ...
      if (parsed.type === "field" && parsed.data) {
        if (
          parsed.data.field_id === deletedFieldId ||
          parsed.data.relationships?.includes(deletedFieldId)
        ) {
          changed = true;
          return "";
        }
 ...
 */
function cleanTiptapContentForDeletedField(
  content: object,
  deletedFieldId: string,
): object | null {
  if (!content || typeof content !== "object") return null;
    ...
    const cleanedNodes = block.content.filter((node: any) => {
      if (node.type === "dynamicVariable" && node.attrs?.variable) {
        ...
        if (fv.type === "field" && fv.data) {
          if (
            fv.data.field_id === deletedFieldId ||
            fv.data.relationships?.includes(deletedFieldId)
          ) {
            changed = true;
            return false;
          }
 ...
 */
function cleanDesignColorForDeletedField(
  dcv: DesignColorVariable,
  deletedFieldId: string,
): DesignColorVariable {
  let changed = false;
  ...
  if (
    result.field?.type === "field" &&
    (result.field.data?.field_id === deletedFieldId ||
      result.field.data?.relationships?.includes(deletedFieldId))
  ) {
    result.field = undefined;
    changed = true;
  }
  ...
  if (result.linear?.stops) {
    const cleanedStops = result.linear.stops.map((stop) => {
      if (
        stop.field?.type === "field" &&
        (stop.field.data?.field_id === deletedFieldId ||
          stop.field.data?.relationships?.includes(deletedFieldId))
      ) {
        changed = true;
        const { field: _, ...rest } = stop;
        return rest;
...
function replaceLayerInTree(
  ...
  return layers.map((layer) => {
    ...
    if (layer.children) {
      const updatedChildren = replaceLayerInTree(
        layer.children,
        layerId,
        replacement,
      );
...
export function updateLayerProps(
  ...
  return layers.map((layer) => {
    ...
    if (layer.children && layer.children.length > 0) {
      return {
        ...layer,
        children: updateLayerProps(layer.children, targetId, props),
      };
    }
    return layer;
  });
...
export function addChildToLayerTree(
  ...
  return layers.map((layer) => {
    ...
    if (layer.children && layer.children.length > 0) {
      return {
        ...layer,
        children: addChildToLayerTree(layer.children, parentId, child),
      };
    }
    return layer;
  });
 ...
 */
export function findLayersWithAnchorId(
  layers: Layer[],
): Array<{ layer: Layer; id: string }> {
