...
import type { CollectionField, InlineVariable } from "@/types/funnel";
...
export function resolveFieldFromSources(
  ...
  if (source === "global") {
    return (
      globalsData?.[fieldId] ??
      collectionItemData?.[fieldId] ??
      pageCollectionItemData?.[fieldId] ??
      undefined
    );
  }
 ...
 */
function buildCanonicalVariable(
  fieldId: string,
  label: string,
 ...
 */
function replaceOutsideCanonical(
  text: string,
  regex: RegExp,
  ...
  while ((m = canonical.exec(text)) !== null) {
    if (m.index > last)
      segments.push({ text: text.slice(last, m.index), isCanonical: false });
    segments.push({ text: m[0], isCanonical: true });
    last = canonical.lastIndex;
  }
  if (last < text.length)
    segments.push({ text: text.slice(last), isCanonical: false });

  return segments
    .map((seg) =>
      seg.isCanonical ? seg.text : seg.text.replace(regex, replacer as any),
    )
    .join("");
}
...
export function normalizeInlineVariableFormats(text: string): string {
  ...
  text = replaceOutsideCanonical(
    ...
    (_full: string, attrs: string) => {
      const fieldId =
        getAttr(attrs, "y_fieldname") || getAttr(attrs, "y_variable") || "";
  ...
  text = replaceOutsideCanonical(
    ...
    (_full: string, attrs: string, fieldId: string) => {
      const explicitLabel =
        getAttr(attrs, "y_name") || getAttr(attrs, "y_fieldname");
      ...
      const label =
        explicitLabel || (looksLikeUuid(fieldId) ? fieldId : "Variable");
      const fieldType = getAttr(attrs, "y_fieldtype") || "text";
      return buildCanonicalVariable(fieldId, label, fieldType);
    },
  );
          ...
          if (depth === 0) {
            ...
            try {
              ...
              if (
                parsed?.type &&
                parsed?.data &&
                (parsed.data.field_id || parsed.data.fieldId)
              ) {
                const fieldId = parsed.data.field_id || parsed.data.fieldId;
                const fieldType =
                  parsed.data.field_type || parsed.data.fieldType || "text";
 ...
 */
function transformOutsideCanonical(
  text: string,
...
export function parseValueToContent(
  ...
  const content: any[] = [];
  const regex =
    /<ycode-inline-variable(?:\s+id="([^"]+)")?>([\s\S]*?)<\/ycode-inline-variable>/g;
  let lastIndex = 0;
  let match;
    ...
    // Priority 2: Parse embedded JSON (legacy format)
      ...
      try {
        ...
        if (parsed.type && parsed.data) {
          ...
          if (
            parsed.label &&
            (resolvedLabel === "field" || resolvedLabel === "[Deleted Field]")
          ) {
            label = parsed.label;
          } else {
            label = resolvedLabel;
