 ...
 */
export function getSvgAspectRatioStyle(
...
export function getSvgIntrinsicSize(
  ...
  if (svgTagMatch) {
    ...
    const widthAttr = attrs.match(
      /\bwidth\s*=\s*["']?\s*([\d.]+)\s*(px)?["'\s>]/i,
    );
    const heightAttr = attrs.match(
      /\bheight\s*=\s*["']?\s*([\d.]+)\s*(px)?["'\s>]/i,
    );
    if (widthAttr && heightAttr) {
...
export function validateCategoryMimeType(
  ...
  const categoryMap: Record<
    string,
    { category: AssetCategory; label: string }
  > = {
    ...
    [ASSET_CATEGORIES.VIDEOS]: {
      category: ASSET_CATEGORIES.VIDEOS,
      label: "video",
    },
    ...
    [ASSET_CATEGORIES.DOCUMENTS]: {
      category: ASSET_CATEGORIES.DOCUMENTS,
      label: "document",
    },
    [ASSET_CATEGORIES.ICONS]: {
      category: ASSET_CATEGORIES.ICONS,
      label: "icon",
    },
  };
 ...
 */
export function getAssetProxyUrl(asset: {
  id: string;
  filename: string;
  mime_type: string;
  ...
  try {
    if (isProxyUrl(url)) {
      ...
      return effectiveSizes
        .map(
          (width) => `${baseUrl}?width=${width}&quality=${quality} ${width}w`,
        )
        .join(", ");
    }
...
export function buildImageSizes(intrinsicWidth: number | null): string {
  return intrinsicWidth
    ? `(max-width: 768px) 100vw, ${intrinsicWidth}px`
    : getImageSizes();
}
 ...
 */
function isSvgAsset(
  asset: { mimeType?: string | null; url?: string | null } | undefined,
): boolean {
  if (!asset) return false;
  if (asset.mimeType && asset.mimeType.toLowerCase().includes("svg"))
    return true;
...
export function findLcpCandidate(
  ...
  resolvedAssets?: Record<
    string,
    { width?: number | null; mimeType?: string | null; url?: string | null }
  >,
  minWidth: number = 200,
): LcpCandidate | null {
  ...
  const visit = (
    ...
    if (layer.name === "image" && !inNonLcp) {
      ...
      const isSvg =
        isSvgAsset(asset) || (inlineUrl ? isSvgUrl(inlineUrl) : false);
...
export function collectLayerAssetIds(
  ...
  const findOverrides = (
    ...
    if (
      node.type === "richTextComponent" &&
      node.attrs?.componentId === targetId
    ) {
    ...
    if (node.type === "richTextComponent" && node.attrs?.componentId) {
      ...
      if (!ancestors.has(cid)) {
        ...
        if (node.attrs._resolvedLayers?.length) {
          (node.attrs._resolvedLayers as any[]).forEach((l) =>
            scanLayer(l, childAncestors),
          );
        } else {
          const comp = components.find((c) => c.id === cid);
          if (comp?.layers?.length) {
            const resolved = applyComponentOverrides(
              comp.layers,
              overrides,
              comp.variables,
            );
