 ...
 */
export function createDynamicTextVariable(
  content: string,
): DynamicTextVariable {
  return {
    type: "dynamic_text",
 ...
 */
export function createDynamicRichTextVariable(
  content: string,
): DynamicRichTextVariable {
  try {
 ...
 */
export function createDynamicRichTextVariableFromPlainText(
  content: string,
): DynamicRichTextVariable {
  return {
    type: "dynamic_rich_text",
 ...
 */
export function createTextComponentVariableValue(
 ...
 * be a variable value.
 */
export function normalizeComponentVariableValue(
  value: unknown,
): ComponentVariableValue | undefined {
 ...
 */
export function extractTiptapFromComponentVariable(
  value?: ComponentVariableValue,
): object {
  ...
  if ("type" in normalized && normalized.type === "dynamic_text") {
    ...
    return stringToTiptapContent(
      (normalized as DynamicTextVariable).data.content,
    );
  }
 ...
 */
export function getDynamicTextContent(
  variable: DynamicTextVariable | undefined | null,
): string {
 ...
 */
export function getStaticTextContent(
  variable: StaticTextVariable | undefined | null,
): string {
  return variable?.data?.content || "";
}
 ...
 */
export function isDynamicTextVariable(
  value: any,
): value is DynamicTextVariable {
  return (
    value &&
    typeof value === "object" &&
    value.type === "dynamic_text" &&
 ...
 */
export function isDynamicRichTextVariable(
  value: any,
): value is DynamicRichTextVariable {
  return (
    value &&
    typeof value === "object" &&
    value.type === "dynamic_rich_text" &&
    value.data?.content !== undefined
  );
}
...
export function resolveDesignColorBinding(
  ...
  if (binding.mode === "linear") {
    const angle =
      modeData && "angle" in modeData
        ? ((modeData as { angle?: number }).angle ?? 90)
        : 90;
...
export function resolveDesignStyles(
  ...
  for (const [designProp, binding] of Object.entries(designBindings)) {
    ...
    if (resolved && cssProp) {
      ...
      const isGradient =
        cssProp === "backgroundColor" && resolved.includes("gradient");
...
import type { LinkSettings, LinkType } from "@/types/funnel";
