...
function paginationDoc(
  parts: Array<{ text: string } | ReturnType<typeof paginationVariableNode>>,
) {
...
export function getPaginationLayerKind(
...
export function hasPaginationVariables(
  ...
  if (
    ...
    typeof textVar.data.content === "string"
  ) {
    return textVar.data.content.includes('"type":"pagination"');
  }
  if (textVar.type === "dynamic_rich_text") {
    return JSON.stringify(textVar.data.content || {}).includes(
      '"type":"pagination"',
    );
  }
  return false;
...
export function resolvePaginationString(
  text: string,
...
export function resolvePaginationDoc(
  doc: unknown,
  numbers: PaginationNumbers,
): unknown {
    ...
    if (node && typeof node === "object") {
      if (
        ...
        return {
          type: "text",
          text: String(
            numbers[node.attrs.variable.data.key as PaginationVariableKey],
          ),
        };
      }
      const next: any = { ...node };
...
export function paginationTextVariableToTemplate(
  ...
  if (
    ...
    typeof textVar.data.content === "object"
  ) {
    let result = "";
...
>(textVar: T, numbers: PaginationNumbers): T {
  if (
    ...
    return {
      ...
      data: {
        ...textVar.data,
        content: resolvePaginationString(textVar.data.content, numbers),
      },
    };
  }
  if (
    ...
    return {
      ...
      data: {
        ...textVar.data,
        content: resolvePaginationDoc(textVar.data.content, numbers),
      },
    };
  }
  return textVar;
