 ...
 */
function remapCollectionLayerIdsInContent(
  content: any,
 ...
 */
function remapVariableCollectionLayerIds(
  vars: LayerVariables,
  idMap: Map<string, string>,
): LayerVariables {
  let changed = false;
  ...
  if (vars.text) {
    ...
    if (
      tv.type === "dynamic_text" &&
      typeof tv.data?.content === "string" &&
      tv.data.content.includes("collection_layer_id")
    ) {
      const newContent = remapCollectionLayerIdsInContent(
        tv.data.content,
        idMap,
      );
      if (newContent !== tv.data.content) {
        result.text = {
          ...tv,
          data: { ...tv.data, content: newContent },
        } as any;
        changed = true;
      }
  ...
  if (vars.backgroundImage?.src) {
    ...
    if (r !== vars.backgroundImage.src) {
      result.backgroundImage = { ...vars.backgroundImage, src: r };
      changed = true;
    }
  }
  ...
  if (vars.link?.field) {
    ...
    if (r !== vars.link.field) {
      result.link = { ...vars.link, field: r };
      changed = true;
    }
  }
  ...
  if (vars.design) {
    ...
    for (const key of [
      ...
      if (vars.design[key]) {
        ...
        if (r !== vars.design[key]) {
          (newDesign as any)[key] = r;
          designChanged = true;
        }
      }
    }
    if (designChanged) {
      result.design = newDesign;
      changed = true;
    }
  }
  ...
  if (vars.conditionalVisibility?.groups?.length) {
    ...
    const newGroups = groups.map((group) => {
      ...
      const newConditions = group.conditions.map((cond) => {
        const mapped = cond?.collectionLayerId
          ? idMap.get(cond.collectionLayerId)
          : undefined;
        return mapped && mapped !== cond.collectionLayerId
          ? { ...cond, collectionLayerId: mapped }
          : cond;
      });
    ...
    if (newGroups.some((g, i) => g !== groups[i])) {
      result.conditionalVisibility = {
        ...vars.conditionalVisibility,
        groups: newGroups,
      };
      changed = true;
    }
...
export function transformLayerIdsForInstance(
  ...
  const transformLayer = (layer: Layer): Layer => {
    ...
    if (layer.variables) {
      transformedLayer.variables = remapVariableCollectionLayerIds(
        layer.variables,
        idMap,
      );
    }
...
type OverrideCategory = Exclude<
  keyof NonNullable<Layer["componentOverrides"]>,
  "variableLinks"
>;
...
export function resolveVariableLinks(
  ...
  for (const [childVarId, parentVarId] of Object.entries(links)) {
    ...
    if (parentVar) {
      ...
      resolvedValue =
        parentOverrides?.[category]?.[parentVarId] ?? parentVar.default_value;
    } else {
      const overrideMatch = findOverrideByVariableId(
        parentOverrides,
        parentVarId,
      );
      if (overrideMatch) {
...
export function applyComponentOverrides(
  ...
  return layers.map((layer) => {
    ...
    if (layer.componentVariantVariableId) {
      ...
      const value = (overrideValue ?? variableDef?.default_value) as
        | VariantSettingsValue
        | undefined;
      if (
        value &&
        ...
        updatedLayer = {
          ...updatedLayer,
          componentVariantId: value.variant_id,
        };
      }
    }
    ...
    if (linkedTextVariableId) {
      const variableDef = componentVariables?.find(
        (v) => v.id === linkedTextVariableId,
      );
      const overrideCategory = (
        variableDef?.type === "rich_text" ? "rich_text" : "text"
      ) as OverrideCategory;
      const overrideValue =
        overrides?.[overrideCategory]?.[linkedTextVariableId];
      const valueToApply = normalizeComponentVariableValue(
        overrideValue ?? variableDef?.default_value,
      );
        ...
        updatedLayer = {
          ...
          variables: {
            ...
            image: {
              ...
              src: imageValue.src
                ? { ...imageValue.src, id: linkedImageVariableId }
                : updatedLayer.variables?.image?.src,
          ...
          ...(overrideValue !== undefined
            ? ({ _imageFromOverride: true } as any)
            : {}),
        };
      }
    }
    ...
    if (linkedLinkVariableId) {
      ...
      if (overrideValue !== undefined || variableDef) {
        ...
        if (linkValue) {
          ...
          const { link: _ignored, ...restVariables } =
            updatedLayer.variables ?? {};
    ...
    if (linkedAudioVariableId) {
      ...
      const variableDef = componentVariables?.find(
        (v) => v.id === linkedAudioVariableId,
      );
      const audioValue = (overrideValue ?? variableDef?.default_value) as any;

      if (audioValue) {
        const audioAttributes: Record<string, unknown> = {};
        if (audioValue.controls !== undefined)
          audioAttributes.controls = audioValue.controls;
        if (audioValue.loop !== undefined)
          audioAttributes.loop = audioValue.loop;
        if (audioValue.muted !== undefined)
          audioAttributes.muted = audioValue.muted;
        if (audioValue.volume !== undefined)
          audioAttributes.volume = String(audioValue.volume);
        ...
        updatedLayer = {
          ...
          variables: {
            ...
            audio: {
              ...
              src: audioValue.src
                ? { ...audioValue.src, id: linkedAudioVariableId }
                : updatedLayer.variables?.audio?.src,
            },
          },
    ...
    if (linkedVideoVariableId) {
      ...
      const variableDef = componentVariables?.find(
        (v) => v.id === linkedVideoVariableId,
      );
      const videoValue = (overrideValue ?? variableDef?.default_value) as any;

      if (videoValue) {
        const videoAttributes: Record<string, unknown> = {};
        if (videoValue.controls !== undefined)
          videoAttributes.controls = videoValue.controls;
        if (videoValue.loop !== undefined)
          videoAttributes.loop = videoValue.loop;
        if (videoValue.muted !== undefined)
          videoAttributes.muted = videoValue.muted;
        if (videoValue.autoplay !== undefined)
          videoAttributes.autoplay = videoValue.autoplay;
        if (videoValue.youtubePrivacyMode !== undefined)
          videoAttributes.youtubePrivacyMode = videoValue.youtubePrivacyMode;
        ...
        updatedLayer = {
          ...
          variables: {
            ...
            video: {
              ...
              src: videoValue.src
                ? { ...videoValue.src, id: linkedVideoVariableId }
                : updatedLayer.variables?.video?.src,
              poster:
                videoValue.poster ?? updatedLayer.variables?.video?.poster,
            },
          },
        ...
        updatedLayer = {
          ...
          variables: {
            ...
            icon: {
              ...
              src: iconValue.src
                ? { ...iconValue.src, id: linkedIconVariableId }
                : updatedLayer.variables?.icon?.src,
            },
          },
        };
      }
    ...
    if (updatedLayer.children) {
      updatedLayer.children = applyComponentOverrides(
        updatedLayer.children,
        overrides,
        componentVariables,
      );
    }
 ...
 */
function tagLayersWithComponentId(
  layers: Layer[],
  componentId: string,
): Layer[] {
  return layers.map((layer) => ({
  ...
  return effectiveLayers.map((layer) => {
    ...
    if (layer.componentId) {
      ...
      if (visited.has(layer.componentId)) {
        console.warn(
          "[resolveComponents] Circular component reference detected, skipping:",
          layer.componentId,
        );
        ...
        if (resolvedInteractions?.length) {
          resolvedInteractions = resolvedInteractions.map((interaction) => ({
            ...
            tweens: interaction.tweens.map((tween) => ({
              ...tween,
              layer_id:
                tween.layer_id === componentContent.id
                  ? layer.id
      ...
      console.warn(
        "[resolveComponents] Component not found or has no layers:",
        layer.componentId,
      );
    }
