import type { Layer, Breakpoint } from "../types/funnel";
 ...
 */
function getOrderValueForBreakpoint(
  layer: Layer,
 ...
 */
function hasAnyOrderClassForBreakpoint(
  layers: Layer[],
  breakpoint: Breakpoint,
): boolean {
  ...
  return layers.some((layer) => {
    ...
    if (breakpoint === "mobile") {
      ...
      return (
        /max-md:order-\d+/.test(classes) || /max-lg:order-\d+/.test(classes)
      );
    }
...
export function getProjection(
  ...
  if (depth > 0) {
    ...
    if (
      dragDepth > 0 &&
      canHaveChildren(overItem.layer) &&
      depth === overItem.depth + 1
    ) {
...
export function findInsertionIndex(
  ...
  const itemsWithSameParent = items.filter(
