 ...
 */
export function isValidLinkSettings(
  link: LinkSettings | undefined | null,
): boolean {
 ...
 */
export function createUrlLinkSettings(
  url: string,
...
export type CollectionItemKeyword =
  (typeof COLLECTION_ITEM_KEYWORDS)[keyof typeof COLLECTION_ITEM_KEYWORDS];
...
export function isCollectionItemKeyword(
  value: string | null | undefined,
): value is CollectionItemKeyword {
  return !!value && KEYWORD_VALUES.has(value);
}

/** Returns true for next/previous page-navigation keywords. */
export function isPageNavigationKeyword(
  value: string | null | undefined,
): boolean {
  return (
    value === COLLECTION_ITEM_KEYWORDS.NEXT_ITEM ||
    value === COLLECTION_ITEM_KEYWORDS.PREVIOUS_ITEM
  );
}
...
export interface LinkResolutionContext {
  ...
  getAsset?: (
    id: string,
  ) => { public_url?: string | null; content?: string | null } | null;
 ...
 */
export function parseCollectionLinkValue(
  value: string | CollectionLinkValue | unknown,
): CollectionLinkValue | null {
...
export interface ResolveFieldLinkOptions {
  ...
  assetMap?: Record<
    string,
    { public_url: string | null; content?: string | null }
  >;
}
...
export function extractCrossCollectionItemIds(
  ...
  for (const item of items) {
    for (const fieldId of linkFieldIds) {
      ...
      if (
        refItemId.startsWith(REF_PAGE_PREFIX) ||
        refItemId.startsWith(REF_COLLECTION_PREFIX)
      )
        continue;
 ...
 */
export function resolveFieldLinkValue(
  options: ResolveFieldLinkOptions,
): string {
...
export function generateLinkHref(
  ...
  switch (linkSettings.type) {
    case "url": {
      ...
      const resolved =
        resolveInlineVariablesFromData(
          urlContent,
          collectionItemData,
          pageCollectionItemData,
        ) || "";
      ...
      href = linkValue
        ? resolveCollectionLinkValue(linkValue, context) || ""
        : resolved;
      break;
    }
    case "email": {
      const emailContent = linkSettings.email?.data?.content || "";
      const resolvedEmail = resolveInlineVariablesFromData(
        emailContent,
        collectionItemData,
        pageCollectionItemData,
      );
    ...
    case "phone": {
      ...
      const resolvedPhone = resolveInlineVariablesFromData(
        phoneContent,
        collectionItemData,
        pageCollectionItemData,
      );
          ...
          if (
            ...
            switch (itemKeyword) {
              case COLLECTION_ITEM_KEYWORDS.CURRENT_PAGE:
                itemSlug = pageCollectionItemId
                  ? collectionItemSlugs[pageCollectionItemId]
                  : undefined;
                break;
              case COLLECTION_ITEM_KEYWORDS.CURRENT_COLLECTION:
                itemSlug = collectionItemId
                  ? collectionItemSlugs[collectionItemId]
                  : undefined;
                break;
              ...
              case COLLECTION_ITEM_KEYWORDS.PREVIOUS_ITEM: {
                ...
                if (pageCollectionSortedItemIds && pageCollectionItemId) {
                  const currentIndex =
                    pageCollectionSortedItemIds.indexOf(pageCollectionItemId);
                  if (currentIndex !== -1) {
                    const offset =
                      itemKeyword === COLLECTION_ITEM_KEYWORDS.NEXT_ITEM
                        ? 1
                        : -1;
                    const targetIndex = currentIndex + offset;
                    if (
                      targetIndex >= 0 &&
                      targetIndex < pageCollectionSortedItemIds.length
                    ) {
                      itemSlug =
                        collectionItemSlugs[
                          pageCollectionSortedItemIds[targetIndex]
                        ];
                    }
                  }
                }
              ...
              default:
                if (
                  itemKeyword.startsWith(REF_PAGE_PREFIX) ||
                  itemKeyword.startsWith(REF_COLLECTION_PREFIX)
                ) {
    ...
    case "field": {
      ...
      let rawValue: string | undefined =
        linkSettings.field?.data?._resolvedValue;
  ...
  if (linkSettings.anchor_layer_id) {
    const anchorValue =
      anchorMap?.[linkSettings.anchor_layer_id] || linkSettings.anchor_layer_id;
    if (href) {
...
function resolveLinkTargetItemId(
  ...
  if (
    collectionItemId.startsWith(REF_PAGE_PREFIX) ||
    collectionItemId.startsWith(REF_COLLECTION_PREFIX)
  ) {
    return resolveRefCollectionItemId(
      collectionItemId,
      context.pageCollectionItemData,
      context.collectionItemData,
    );
  }
...
function getCurrentPagePath(context: LinkResolutionContext): string | null {
  ...
  const selfLink = createPageLinkSettings(
    context.pageId,
    context.pageCollectionItemId ?? null,
  );
