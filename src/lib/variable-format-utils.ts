...
import type { CollectionFieldType } from "@/types/funnel";
import {
  isDateFieldType,
  isDateOnlyString,
...
export const DATE_ONLY_FORMAT_SECTIONS: FormatPresetSection<DateFormatPreset>[] =
  DATE_FORMAT_SECTIONS.filter(
    (s) => s.title === "Date" || s.title === "Date parts",
  );
...
export function isFormattableFieldType(
...
export function getDefaultFormatId(
  fieldType: string | null | undefined,
): string | undefined {
...
export function formatDateWithPreset(
  ...
  const effectiveTimezone =
    typeof value === "string" && isDateOnlyString(value) ? "UTC" : timezone;

  const preset = presetId
    ? datePresetMap.get(presetId)
    : datePresetMap.get("date-long");
  if (!preset) {
  ...
  try {
    ...
    const result = preset.extractPart
      ? (formatter
          ...
          .find((p) => p.type === preset.extractPart)?.value ?? "")
