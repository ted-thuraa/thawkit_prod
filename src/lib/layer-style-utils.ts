...
import type { Layer, LayerStyle, TextStyle } from "@/types/funnel";
 ...
 */
function buildDesignPreservingBgVars(
  classes: string,
  source: Pick<Layer, "design">,
): Layer["design"] {
  const design = buildDesign(classes);
  const bg = source.design?.backgrounds;
  if (!design || !bg) return design;
  const grad =
    bg.bgGradientVars && Object.keys(bg.bgGradientVars).length > 0
      ? bg.bgGradientVars
      : undefined;
  const img =
    bg.bgImageVars && Object.keys(bg.bgImageVars).length > 0
      ? bg.bgImageVars
      : undefined;
...
export function detachStyleFromLayer(layer: Layer, style?: LayerStyle): Layer {
  ...
  // the rendered look.
  const { styleId, styleIds, styleOverrides, styleOverridesByStyle, ...rest } =
    layer;
...
export function updateStyledLayer(
  ...
  return {
    ...
    styleOverrides: {
      classes:
        updates.classes !== undefined
          ? updates.classes
          : layer.styleOverrides?.classes,
      design:
        updates.design !== undefined
          ? updates.design
          : layer.styleOverrides?.design,
    },
  };
}
  ...
  return layers.map((layer) => {
    ...
    if (ids.includes(changedStyleId) && !layer.styleOverrides) {
      ...
      updatedLayer = {
        ...updatedLayer,
        classes,
        design: buildDesignPreservingBgVars(classes, layer),
      };
    }

    // Update textStyles entries that reference this style (single-style only)
    if (layer.textStyles) {
      const updatedTextStyles = updateTextStylesWithStyle(
        layer.textStyles,
        changedStyleId,
        newClasses,
        newDesign,
    ...
    if (layer.children && layer.children.length > 0) {
      const updatedChildren = updateLayersWithStyle(
        layer.children,
        changedStyleId,
        stylesById,
      );
...
export function detachStyleFromLayers(
  ...
  return layers.map((layer) => {
    ...
    if (layer.textStyles) {
      const updatedTextStyles = detachStyleFromTextStyles(
        layer.textStyles,
        removedStyleId,
      );
      if (updatedTextStyles) {
    ...
    if (layer.children && layer.children.length > 0) {
      const updatedChildren = detachStyleFromLayers(
        layer.children,
        removedStyleId,
        stylesById,
      );
 ...
 */
export function countLayersUsingStyle(
  layers: Layer[],
  styleId: string,
): number {
  let count = 0;
 ...
 */
export function getLayerIdsUsingStyle(
  layers: Layer[],
  styleId: string,
): string[] {
  const ids: string[] = [];
