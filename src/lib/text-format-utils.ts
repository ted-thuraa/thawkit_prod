 ...
 */
function refreshGlobalVariableNodes<T>(
  node: T,
...
export const DEFAULT_TEXT_STYLES: Record<string, TextStyle> = {
  ...
  table: {
    label: "Table",
    classes:
      "w-full border-separate border-spacing-0 mt-[20px] mb-[20px] border-solid border-[1px] border-[#000000]/10 rounded-[10px] overflow-hidden",
    design: {
      sizing: { width: "100%" },
  ...
  tableHeader: {
    ...
    classes:
      "text-left font-[500] pt-[12px] pr-[12px] pb-[12px] pl-[12px] bg-[#000000]/5",
    design: {
...
function getVariableNodeData(
  ...
  if (
    node.attrs?.variable?.type === "field" &&
...
function resolveVariableNode(
  ...
  const { fieldType, rawValue, format } = getVariableNodeData(
    node,
    collectionItemData,
    pageCollectionItemData,
  );
...
function renderTextNode(
  ...
  const getMarkClass = (markKey: string) =>
    getTextStyleClasses(textStyles, markKey);
      ...
      switch (mark.type) {
        case "bold":
          text = React.createElement(
            "strong",
            buildProps("bold", getMarkClass("bold")),
            text,
          );
          break;
        case "italic":
          text = React.createElement(
            "em",
            buildProps("italic", getMarkClass("italic")),
            text,
          );
          break;
        case "underline":
          text = React.createElement(
            "u",
            buildProps("underline", getMarkClass("underline")),
            text,
          );
          break;
        case "strike":
          text = React.createElement(
            "s",
            buildProps("strike", getMarkClass("strike")),
            text,
          );
          break;
        case "subscript":
          text = React.createElement(
            "sub",
            buildProps("subscript", getMarkClass("subscript")),
            text,
          );
          break;
        case "superscript":
          text = React.createElement(
            "sup",
            buildProps("superscript", getMarkClass("superscript")),
            text,
          );
          break;
        case "dynamicStyle": {
  ...
  return content
    .flatMap((node, idx) => {
      ...
      if (node.type === "richTextComponent" && node.attrs?.componentId) {
        const rendered = renderRichTextComponentBlock(
          node,
          key,
          components,
          renderComponentBlock,
      ...
      if (node.type === "richTextImage" || node.type === "image") {
        ...
        const imgLinkHref = resolveImageLinkHref(
          node.attrs,
          isEditMode,
          linkContext,
          collectionItemData,
...
export type RenderComponentBlockFn = (
  component: Component,
  resolvedLayers: import("@/types/funnel").Layer[],
  overrides: import("@/types/funnel").Layer["componentOverrides"],
  key: string,
  ancestorComponentIds?: Set<string>,
) => React.ReactNode;
...
function renderRichTextComponentBlock(
  ...
  if (block.attrs._resolvedLayers) {
    ...
    const resolvedComponent =
      component ?? ({ id: componentId, name: "" } as Component);
    if (!renderComponentBlock) {
      return React.createElement(
        "span",
        { key, "data-component-id": componentId },
        `[${resolvedComponent.name}]`,
      );
    }
    return renderComponentBlock(
      resolvedComponent,
      block.attrs._resolvedLayers,
      overrides,
      key,
      updatedAncestors,
    );
  ...
  if (!component) {
    return React.createElement(
      "span",
      { key, className: "text-xs text-muted-foreground" },
      "[missing component]",
    );
  }
...
function renderBlock(
  ...
  if (block.type === "paragraph") {
    ...
    const hasBlockContent = block.content?.some(
      (n: any) =>
        (n.type === "dynamicVariable" &&
          n.attrs?.variable?.data?.field_type === "rich_text") ||
  ...
  if (block.type === "richTextComponent" && block.attrs?.componentId) {
    return renderRichTextComponentBlock(
      block,
      key,
      components,
      renderComponentBlock,
...
function renderTableNode(
  ...
  if (
    ...
    if (node.attrs.rowspan && node.attrs.rowspan > 1)
      props.rowSpan = node.attrs.rowspan;
  }
...
export function hasBlockElementsWithInlineVariables(
  ...
  const resolveValue = (
    ...
    return (
      collectionItemData?.[lookupKey] ?? pageCollectionItemData?.[lookupKey]
    );
  };
...
export function renderRichText(
  ...
  const content = refreshGlobalVariableNodes(
  ...
  if (doc.content.length === 1 && doc.content[0].type === "paragraph") {
    ...
    if (!isSimpleTextElement) {
      ...
      const paragraphClass =
        ...
        DEFAULT_TEXT_STYLES.paragraph?.classes ??
        "";
      const children = Array.isArray(inlineContent)
        ? inlineContent
        : [inlineContent];
      const props: Record<string, any> = {
        className: paragraphClass || undefined,
      };
      if (isEditMode) {
  ...
  return doc.content.map((block: any, idx: number) => {
    ...
    const isFragment =
      ...
      (element as React.ReactElement).type === React.Fragment;
